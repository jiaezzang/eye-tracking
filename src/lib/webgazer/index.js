...
function checkEyesInValidationBox() {
    if (faceFeedbackBox != null && latestEyeFeatures) {
        ...
        var eyeLW = latestEyeFeatures.left.width;
        var eyeRW = latestEyeFeatures.right.width;
        ...
        /** 왼쪽 눈과 오른쪽 눈의 크기 비율 */
        const eyesSizeRatio = eyeRW > eyeLW ? eyeLW / eyeRW : eyeRW / eyeLW;
        const limit = eyesSizeRatio > 0.7;
        ...
        if (xPositions && yPositions && limit) {
        ...
        if (!limit) gazeDot.style.background = 'green';
    } else {
...
var recordScreenMultiplePosition = function (x, y) {
    const offset = [-2, -1, 0, 1, 2].map((offset) => offset * 5);

    for (let i = 0; i < offset.length; i++) {
        for (let j = 0; j < offset.length; j++) {
            webgazer.recordScreenPosition(Math.max(x + offset[i], 0), Math.max(y + offset[j], 0), 'click');
        }
    }
};
...
async function setGlobalData() {
    ...
    // localstorageDataLabel key를 모두 가져옴
    // const storedLabels = await localforage
    //   .getItem(localstorageDataLabel)
    ...
    //데이터를 110 미만으로 쌓도록 조건화
    // if (regs.length < 110) localforage.setItem(localstorageDataLabel, storeData);
...
/**
 * 웹캠 내 학습자의 얼굴이 중앙으로부터 얼마나 가까운지 확인할 수 있는 box color
 * @returns green, red, black
 */
webgazer.faceFeedbackBoxBorder = () => {
    return faceFeedbackBox.style.border.replace('solid ', '');
};

/**
 * 학습자가 카메라 중앙에서 화면을 제대로 응시하고 있는지 확인할 수 있는 gazeDot의 color
 * @returns green, red, black
 */
webgazer.gazeDotColor = () => {
    return gazeDot.style.background;
};

/**
 * IndexedDB에 GlobalData 저장
 */
...
/** regs 초기값 등록 */
webgazer.setRegsInit = () => {
    if (regs.length === 0) {
        regs[0].addData(latestEyeFeatures, [window.innerWidth / 2, window.innerHeight / 2], 'click');
        console.log('yes');
    }
};

/** IndexedDB GlobalData length */
webgazer.getGlobalDataLength = () => {
    return regs[0].dataClicks.data.length;
};

webgazer.recordScreenMultiplePosition = (x, y) => {
    recordScreenMultiplePosition(x, y);
};

console.log('loaded webgazer');
